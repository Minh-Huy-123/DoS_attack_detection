...
public class BasicFlow {
	...
	public String getLabel() {
		...
		Model model = null;
		try {
			model = Model.fromFile("models/rf_DoS_classifier_mdx20.pmml");
		} catch (Exception e) {
...
//			put("Packet Length Max", getMaxPacketLength());
//			put("Bwd Init Win Bytes", getInit_Win_bytes_backward());
//			put("Bwd Packet Length Max", getBwdPacketLengthMax());
//			put("Total Length of Bwd Packet", getTotalLengthofBwdPackets());
//			put("Total Bwd packets", getTotalBackwardPackets());
...
//			put("Total Length of Fwd Packet", getTotalLengthofFwdPackets());
...
//			put("Subflow Bwd Bytes", getSflow_bbytes());
			...
			put("Fwd Seg Size Avg", fAvgSegmentSize());
			...
			put("Bwd Segment Size Avg", bAvgSegmentSize());
			put("Fwd Packet Length Mean", getFwdPacketLengthMean());
			put("Fwd Pkt Len Mean", getFwdPacketLengthMean());
			put("Fwd Seg Size Min", getmin_seg_size_forward());
			put("Total Fwd packets", getTotalFwdPackets());

			put("FWD Init Win Bytes", getInit_Win_bytes_forward());
			put("Packet Length Mean", getPacketLengthMean());
			put("Packet Length Max", getMaxPacketLength());
			put("SYN Flag Count", flagCounts.get("SYN").value);
			put("Bwd Header Length", getBwdHeaderLength());
			put("Average packet size", getAvgPacketSize());
			put("Fwd Header Length", getFwdHeaderLength());
			put("Bwd Packet Length Max", getBwdPacketLengthMax());
			put("Fwd Packet Length Min", getFwdPacketLengthMin());
			put("Fwd Act Data Pkts", getAct_data_pkt_forward());
			put("Total length of Bwd packet", getTotalLengthofBwdPackets());

			put("Flow IAT Min", flowIAT.getMin());
			put("Flow IAT Mean", flowIAT.getMean());

		}});

//		Double dosProbability = (Double) result.getOrDefault("probability(DoS Attack)", 0.0);
//		Double	dosScore = Math.min(dosProbability + 0.2, 1.0);
//		dosScore = Math.round(dosScore * 100.0) / 100.0;
//		double benignScore = Math.round((1 - dosScore) * 100.0) / 100.0;
//
//		String label = dosScore > 0.5 ? "probability(DoS Attack)" : "probability(BENIGN)";
//
//		System.out.println(flowId.toString()
//				+ " {probability(DoS Attack)= " + dosScore
//				+ ", probability(BENIGN)=" + benignScore + "}"
//				+ " ==> Label: " + label);
//
//		return label;
		...
		Double dosProbability = (Double) result.getOrDefault("probability(DoS Attack)", 0.0);
		String label = dosProbability > 0.5 ? "probability(DoS Attack)" : "probability(BENIGN)";
		System.out.println(flowId.toString() + result.toString() + "==> Label: " + label);
		return label;
